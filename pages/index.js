...
// Electrical labor rates (can be customized)
const LABOR_RATE = 95 // per hour
const TRIP_CHARGE = 75

// Service categories with typical costs
const SERVICE_TYPES = [
  'Panel Upgrade',
  'Circuit Addition',
  'Outlet Replacement',
...
export default function QuoteSnap() {
  const [step, setStep] = useState('upload') // upload | details | analyzing | quote
  const [imageData, setImageData] = useState(null) // { dataUrl, width, height, filename }
  ...
  const processFile = useCallback(async (file) => {
    ...
    try {
      ...
      if (isHeic || file.type === 'image/heic') {
        try { blob = await convertHeic(file) } catch { /* proceed with original */ }
      }
      const { blob: resizedBlob, dataUrl, width, height } = await resizeImage(blob)
  ...
  const handleTouchMove = (e) => {
    if (e.touches.length === 2) {
      ...
      if (touchStart) {
        const scale = Math.min(4, Math.max(1, pinchScale * (dist / touchStart)))
        setPinchScale(scale)
      }
    } else if (e.touches.length === 1 && pinchScale > 1) {
      setPan({
  ...
  const s = {
    ...
    quoteActionBtn: (primary) => ({
      ...
      background: primary ? 'linear-gradient(135deg, #f59e0b, #d97706)' : '#13131f',
      color: primary ? '#000' : '#f0f0f5',
    ...
    <div style={s.app}>
      <Head>
        ...
        <style>{`
          ...
          * { box-sizing: border-box; -webkit-tap-highlight-color: transparent; }
          ...
          .upload-zone:hover { border-color: #f59e0b66 !important; }
      ...
      <main style={s.main}>
          ...
          <div style={s.uploadWrap} onDrop={handleDrop} onDragOver={(e) => e.preventDefault()}>
            ...
            <div style={{ maxWidth: '360px' }}>
              ...
              <p style={s.uploadSub}>
                ...
                AI will analyze it and build a detailed quote.
              </p>
            </div>
            ...
            <label
              ...
              style={{ ...s.dropZone, display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '12px' }}
            >
        ...
        {step === 'details' && imageData && (
          <>
            <div style={s.jobInfoBar}>
              ...
              <span style={{ flex: 1 }}>{imageData.filename}</span>
              <span style={s.jobInfoTag}>{imageData.width}×{imageData.height}</span>
            </div>

            <div style={s.detailsWrap} ref={detailsFormRef}>
              <div>
              ...
              <div>
                ...
                <textarea
                  style={s.textarea}
                  placeholder="Describe the issue, access notes, special requirements…"
                  value={notes}
              ...
              <button className="back-btn" style={s.backBtn} onClick={startOver}>
        ...
        {step === 'analyzing' && (
          <div style={s.analyzingWrap}>
            ...
            <p style={s.analyzingSub}>
              AI is reading the image and building your quote.<br />
        ...
        {step === 'quote' && quoteResult && (
          <div style={s.quoteWrap}>
            <div style={s.quoteTabs}>
              <div
                style={s.quoteTab(activeTab === 'preview')}
                onClick={() => setActiveTab('preview')}
              >
                📷 Photo
              </div>
              <div
                style={s.quoteTab(activeTab === 'quote')}
            ...
            {activeTab === 'preview' ? (
              <div
                ref={containerRef}
                style={{ flex: 1, overflow: 'hidden', position: 'relative', touchAction: 'none', background: '#000' }}
                ...
                {pinchScale > 1 && (
                  <button
                    ...
                    style={{
                      ...
                      background: '#13131fcc', color: '#fff', border: 'none',
              ...
              <div style={s.quoteContent} ref={quoteRef}>
                <div style={s.quoteCard}>
                  <div style={s.quoteCardHeader}>
                    <div style={s.quoteCardTitle}>⚡ Electrical Quote</div>
                    <div style={s.quoteCardMeta}>
                      {customerName || 'Customer'} · {serviceType}
                      {customerAddress ? ` · ${customerAddress}` : ''}
                    </div>
                    <div style={{ ...s.quoteCardMeta, marginTop: '2px' }}>
                    ...
                    <div key={i} style={s.lineItem}>
                      <div style={s.lineItemDesc}>
                        ...
                        <div style={s.lineItemDetail}>
                          {item.qty > 1 ? `${item.qty}× ` : ''}{item.laborHrs ? `${item.laborHrs}h labor · ` : ''}{item.materials ? item.materials : ''}
                        </div>
                      </div>
                  ...
                  <div style={s.subtotalRow}>
                    ...
                    <span>${TRIP_CHARGE.toFixed(2)}</span>
                  </div>
                  <div style={s.subtotalRow}>
                    <span>Labor ({quoteResult.totalHours}h @ ${LABOR_RATE}/hr)</span>
                    <span>${(quoteResult.totalHours * LABOR_RATE).toFixed(2)}</span>
                  </div>
                  <div style={s.subtotalRow}>
                    <span>Materials</span>
                    <span>${(quoteResult.materialsTotal || 0).toFixed(2)}</span>
                  </div>
                  ...
                  {quoteResult.warranties && (
                    <div style={s.warrantyNote}>
                      <strong style={{ color: '#9ca3af' }}>Warranty:</strong> {quoteResult.warranties}
                    </div>
                  )}
                </div>

                <p style={{ fontSize: '12px', color: '#4b5563', marginTop: '16px', lineHeight: '1.6' }}>
                  This is an estimate. Final price may vary based on actual conditions found on-site.
                  Valid for 30 days.
                </p>
              </div>
            )}
      ...
      {step === 'quote' && (
        <footer style={{ padding: '12px 20px', borderTop: '1px solid #1e1e2e', fontSize: '12px', color: '#4b5563', textAlign: 'center', flexShrink: 0 }}>
          Mallard Electric LLC · Quote Snap v1.0
        </footer>
      )}
    </div>
  )
